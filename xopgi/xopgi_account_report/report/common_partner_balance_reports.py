...
# -*- encoding: utf-8 -*-
# ---------------------------------------------------------------------
# common_partner_balance_reports
# ---------------------------------------------------------------------
...
class CommonPartnerBalanceReportHeaderWebkit(CommonBalanceReportHeaderWebkit,
        ...
        for account_id, account_details in account_by_ids.iteritems():
            ...
            if partners_init_balances_by_ids.get(account_id):
                for partner_id, initial_balances in \
                    ...
                    if initial_balances.get('init_balance_currency'):
                        details[partner_id].update(
                            {'init_balance_currency': initial_balances['init_balance_currency']})
            ...
            for partner_id, partner_details in details.iteritems():
                ...
                details[partner_id]['balance_currency'] = details[partner_id].\
                    get('init_balance_currency', 0.0) + \
                    details[partner_id].get('currency_debit', 0.0) - \
                    details[partner_id].get('currency_credit', 0.0)
    ...
    def _get_partners_totals_account(self, filter_from, account_id, start,
        ...
        sql_select = """
                 SELECT account_move_line.partner_id,
                        ...
                        sum(account_move_line.currency_debit) as currency_debit,
                        sum(account_move_line.currency_credit) as currency_credit,
                        a.currency_id as currency_id,
                        c.name as currency_name
                 FROM account_move_line"""

        sql_joins = " INNER JOIN account_account a \
                      ON a.id = account_move_line.account_id \
        ...
        sql_groupby = "GROUP BY account_move_line.partner_id, a.currency_id, c.name "
    ...
    def compute_partner_balance_data(self, data, filter_report_type=None):
        ...
        currency_company = {}
        currency_company = self._get_currency_company()
    ...
    def _get_currency_company(self):
        ...
        sql_select = """
                 SELECT res_company.currency_id,
                        c.name as currency_name
                 FROM res_company
                 INNER JOIN res_currency c
                 ON c.id = res_company.currency_id"""
